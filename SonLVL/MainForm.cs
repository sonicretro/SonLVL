	...
	public partial class MainForm : Form
	{
		...
		private void backgroundLevelLoader_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			...
			if (LevelData.Palette.Count > 1)
			{
				...
				LevelData.WaterPalette = LevelData.Level.WaterPalette < 2 ? -1 : LevelData.Level.WaterPalette - 1;
			...
			ChunkCount.Text = LevelData.Chunks.Count.ToString("X") + " / " + LevelData.GetChunkMax().ToString("X");
				...
				foreach (LayoutCopyInfo area in LevelData.Level.LayoutCopy.Sections)
					...
					else
					{
						for (int y = 0; y < area.Height; y++)
							for (int x = 0; x < area.Width; x++)
							{
								ushort tmp = LevelData.Layout.FGLayout[area.X + x, area.Y + y];
		...
		private void paletteToolStripDropDownButton_DropDownItemClicked(object sender, ToolStripItemClickedEventArgs e)
		{
			...
			if (LevelData.WaterPalette == LevelData.CurPal)
				LevelData.WaterPalette = -1;
			selectPaletteToolStripMenuItem.DropDownItems.Clear();
			selectPaletteToolStripMenuItem.DropDownItems.Add(new ToolStripMenuItem("None") { Checked = LevelData.WaterPalette == -1, Tag = -1 });
			for (int i = 0; i < LevelData.PalName.Count; i++)
				if (i != LevelData.CurPal)
					selectPaletteToolStripMenuItem.DropDownItems.Add(new ToolStripMenuItem(LevelData.PalName[i]) { Checked = LevelData.WaterPalette == i, Tag = i });
				...
				if (a.ShowDialog() == DialogResult.OK)
				{
					if (exportArtcollisionpriorityToolStripMenuItem.Checked)
					{
						...
						for (int i = 0; i < bmp.Bits.Length; i++)
							if (bmp.Bits[i] == 0)
								bmp.Bits[i] = 32;
						if (LevelData.WaterPalette != -1 && bmp.Height > LevelData.WaterHeight)
							bmp.ApplyWaterPalette(LevelData.WaterHeight);
		...
		internal void DrawLevel()
		{
			...
			switch (CurrentTab)
			{
				...
				case Tab.Foreground:
					...
					if (LevelData.WaterPalette != -1 && camera.Y + LevelImg8bpp.Height > LevelData.WaterHeight)
						LevelImg8bpp.ApplyWaterPalette(Math.Max(LevelData.WaterHeight - camera.Y, 0));
					break;
				case Tab.Background:
		...
		private void foregroundPanel_KeyDown(object sender, KeyEventArgs e)
		{
			...
			switch (e.KeyCode)
			{
				...
				case Keys.A:
					if (!loaded) return;
					SelectedChunk = (ushort)(SelectedChunk == 0 ? LevelData.Chunks.Count - 1 : SelectedChunk - 1);
					DrawLevel();
					break;
				case Keys.Z:
					if (!loaded) return;
		...
		private void ChunkSelector_MouseDown(object sender, MouseEventArgs e)
		{
			...
			{
				int chunkmax = LevelData.GetChunkMax();
				...
				pasteBeforeToolStripMenuItem.Enabled = pasteOverToolStripMenuItem.Enabled && LevelData.Chunks.Count < chunkmax;
				pasteAfterToolStripMenuItem.Enabled = pasteBeforeToolStripMenuItem.Enabled;
				insertAfterToolStripMenuItem.Enabled = LevelData.Chunks.Count < chunkmax;
		...
		private void DeleteChunk()
		{
			...
			ChunkID.Maximum = LevelData.Chunks.Count - 1;
			SelectedChunk = (ushort)Math.Min(SelectedChunk, LevelData.Chunks.Count - 1);
		...
		private bool ImportImage(Bitmap bmp, Bitmap colbmp1, Bitmap colbmp2, Bitmap pribmp, ushort[,] layout)
		{
			...
			if (newChunks.Count > 0)
			{
				...
				SelectedChunk = (ushort)(LevelData.Chunks.Count - 1);
				ChunkID.Maximum = LevelData.Chunks.Count - 1;
				ChunkSelector.SelectedIndex = SelectedChunk;
			}
		...
		private void ImportChunk(PatternIndex[,] map, BlockColInfo[,] blockcoldata, List<byte[]> blocks, List<byte[]> chunks, List<byte> colInds1, List<byte> colInds2, List<Block> newBlocks, List<Chunk> newChunks, List<byte> newColInds1, List<byte> newColInds2, ushort[,] layout, int cx, int cy)
		{
			Chunk cnk = new Chunk();
				...
				switch (e.KeyCode)
				{
					...
					case Keys.D:
						if (e.Control)
							...
							{
								case ArtTab.Chunks:
									if (LevelData.Chunks.Count < LevelData.GetChunkMax())
					...
					case Keys.V:
						if (e.Control)
							...
							{
								case ArtTab.Chunks:
									if ((Clipboard.ContainsData(typeof(ChunkCopyData).AssemblyQualifiedName) || Clipboard.ContainsData(typeof(Chunk).AssemblyQualifiedName)) && LevelData.Chunks.Count < LevelData.GetChunkMax())
		...
		private void pasteOnceToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			if (CurrentTab == Tab.Background)
			{
				BGSelection = new Rectangle(BGSelection.X, BGSelection.Y, section.Layout.GetLength(0), section.Layout.GetLength(1));
				BGSelection.Width = Math.Min(BGSelection.Right, LevelData.BGWidth) - BGSelection.Left;
				BGSelection.Height = Math.Min(BGSelection.Bottom, LevelData.BGHeight) - BGSelection.Top;
			}
			else
			{
				FGSelection = new Rectangle(FGSelection.X, FGSelection.Y, section.Layout.GetLength(0), section.Layout.GetLength(1));
		...
		private void PasteLayoutSectionOnce(LayoutSection section)
		{
			...
			if (CurrentTab == Tab.Background)
			{
				...
				xflip = LevelData.Layout.BGXFlip;
				yflip = LevelData.Layout.BGYFlip;
				location = new Point(BGSelection.X, BGSelection.Y);
				w = Math.Min(section.Layout.GetLength(0), LevelData.BGWidth - location.X);
				h = Math.Min(section.Layout.GetLength(1), LevelData.BGHeight - location.Y);
			}
			else
			{
				layout = LevelData.Layout.FGLayout;
				...
				xflip = LevelData.Layout.FGXFlip;
				yflip = LevelData.Layout.FGYFlip;
				location = new Point(FGSelection.X, FGSelection.Y);
				w = Math.Min(section.Layout.GetLength(0), LevelData.FGWidth - location.X);
				h = Math.Min(section.Layout.GetLength(1), LevelData.FGHeight - location.Y);
			}
			for (int y = 0; y < h; y++)
				for (int x = 0; x < w; x++)
				{
					layout[x + location.X, y + location.Y] = section.Layout[x, y];
					if (loop != null)
					{
						if (section.Loop == null)
							loop[x + location.X, y + location.Y] = false;
						else
							loop[x + location.X, y + location.Y] = section.Loop[x, y];
					}
					if (xflip != null)
					{
						if (section.XFlip == null)
							xflip[x + location.X, y + location.Y] = false;
						else
							xflip[x + location.X, y + location.Y] = section.XFlip[x, y];
					}
					if (yflip != null)
					{
						if (section.YFlip == null)
							yflip[x + location.X, y + location.Y] = false;
						else
							yflip[x + location.X, y + location.Y] = section.YFlip[x, y];
					}
				}
			if (CurrentTab == Tab.Foreground) DoLayoutCopy();
			if (includeObjectsWithForegroundSelectionToolStripMenuItem.Checked & CurrentTab == Tab.Foreground)
			{
				Size off = new Size(location.X * LevelData.Level.ChunkWidth, location.Y * LevelData.Level.ChunkHeight);
			...
			for (int y = selection.Top; y < selection.Bottom; y++)
				for (int x = selection.Left; x < selection.Right; x++)
				{
					...
					if (loop != null)
						loop[x, y] = false;
					if (xflip != null)
						xflip[x, y] = chunkXFlip.Checked;
					if (yflip != null)
						yflip[x, y] = chunkYFlip.Checked;
				}
			if (CurrentTab == Tab.Foreground) DoLayoutCopy();
			DrawLevel();
		}
		...
		private void remapChunksButton_Click(object sender, EventArgs e)
		{
			using (TileRemappingDialog dlg = new TileRemappingDialog("Chunks", LevelData.CompChunkBmps, ChunkSelector.ImageWidth, ChunkSelector.ImageHeight))
				if (dlg.ShowDialog(this) == DialogResult.OK)
				{
					...
					{
						LevelData.Chunks[item.Value] = oldchunks[item.Key].Clone();
						LevelData.ChunkSprites[item.Value] = oldchunkbmpbits[item.Key];
						LevelData.ChunkBmps[item.Value].CopyTo(oldchunkbmps[item.Key], 0);
						LevelData.ChunkColBmpBits[item.Value].CopyTo(oldchunkcolbmpbits[item.Key], 0);
						LevelData.ChunkColBmps[item.Value].CopyTo(oldchunkcolbmps[item.Key], 0);
		...
		private void remapBlocksButton_Click(object sender, EventArgs e)
		{
			using (TileRemappingDialog dlg = new TileRemappingDialog("Blocks", LevelData.CompBlockBmps, BlockSelector.ImageWidth, BlockSelector.ImageHeight))
				if (dlg.ShowDialog(this) == DialogResult.OK)
				{
					...
					foreach (KeyValuePair<int, int> item in dlg.TileMap)
					{
						LevelData.Blocks[item.Value] = oldblocks[item.Key].Clone();
						LevelData.BlockBmpBits[item.Value].CopyTo(oldblockbmpbits[item.Key], 0);
						LevelData.BlockBmps[item.Value].CopyTo(oldblockbmps[item.Key], 0);
			...
			using (TileRemappingDialog dlg = new TileRemappingDialog("Tiles", TileSelector.Images, TileSelector.ImageWidth, TileSelector.ImageHeight))
				if (dlg.ShowDialog(this) == DialogResult.OK)
				{
					...
					foreach (KeyValuePair<int, int> item in dlg.TileMap)
					{
						if (LevelData.Level.TwoPlayerCompatible)
						{
							oldtiles[item.Key * 2].CopyTo(LevelData.Tiles[item.Value * 2], 0);
							oldtiles[item.Key * 2 + 1].CopyTo(LevelData.Tiles[item.Value * 2 + 1], 0);
						...
						{
							oldtiles[item.Key].CopyTo(LevelData.Tiles[item.Value], 0);
							TileSelector.Images[item.Value] = oldimages[item.Key];
							ushortdict.Add((ushort)item.Key, (ushort)item.Value);
						}
					}
			...
			using (OpenFileDialog opendlg = new OpenFileDialog()
			{
				...
				if (opendlg.ShowDialog(this) == DialogResult.OK)
					using (Bitmap bmp = new Bitmap(opendlg.FileName))
					{
						...
						if (File.Exists(string.Format(fmt, "pri")))
							pribmp = new Bitmap(string.Format(fmt, "pri"));
						ushort[,] section = new ushort[bmp.Width / LevelData.Level.ChunkWidth, bmp.Height / LevelData.Level.ChunkHeight];
		...
		private void selectPaletteToolStripMenuItem_DropDownItemClicked(object sender, ToolStripItemClickedEventArgs e)
		{
			foreach (ToolStripMenuItem item in selectPaletteToolStripMenuItem.DropDownItems)
				item.Checked = false;
			((ToolStripMenuItem)e.ClickedItem).Checked = true;
			LevelData.WaterPalette = (int)e.ClickedItem.Tag;
			selectPaletteToolStripMenuItem.DropDownItems.Clear();
			selectPaletteToolStripMenuItem.DropDownItems.Add(new ToolStripMenuItem("None") { Checked = LevelData.WaterPalette == -1, Tag = -1 });
			for (int i = 0; i < LevelData.PalName.Count; i++)
				if (i != LevelData.CurPal)
		...
		{
			using (WaterHeightDialog dlg = new WaterHeightDialog(LevelData.WaterHeight))
				if (dlg.ShowDialog(this) == DialogResult.OK)
				{
					LevelData.WaterHeight = dlg.Value;
					DrawLevel();
				}
		}
		...
		private void deleteUnusedChunksToolStripButton_Click(object sender, EventArgs e)
		{
			...
			ChunkID.Maximum = LevelData.Chunks.Count - 1;
			ChunkSelector.SelectedIndex = SelectedChunk = Math.Min(SelectedChunk, (ushort)(LevelData.Chunks.Count - 1));
		...
		private void removeDuplicateChunksToolStripButton_Click(object sender, EventArgs e)
		{
			if (MessageBox.Show(this, "This action may break other levels that share the same chunk set, or levels that alter the level layout dynamically.\n\nAre you sure you want to remove all duplicate chunks?", "SonLVL", MessageBoxButtons.OKCancel) != DialogResult.OK)
				return;
			Dictionary<ushort, byte[]> chunks = new Dictionary<ushort, byte[]>(LevelData.Chunks.Count);
			Dictionary<ushort, ushort> chunkMap = new Dictionary<ushort, ushort>(LevelData.Chunks.Count);
